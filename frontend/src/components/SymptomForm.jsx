...
import apiClient from '../api'; // Assuming you have api.js for axios
...
export default function SymptomForm({ demographics, onResult, onBack }) {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6"> {/* RESPONSIVE CHANGE: Smaller padding on mobile */}
      <div>
        {/* The heading now has responsive text size */}
        <h2 className="text-xl md:text-2xl font-semibold text-white"> {/* RESPONSIVE CHANGE: Smaller text on mobile */}
      ...
      <div className="mt-4">
        ...
        >
          ...
          <svg className="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 16v-8m0 0l-3 3m3-3l3 3M4 16v4h16v-4" />
          </svg>
          Upload Blood Report (PDF)
        </label>
        <input id="file-upload" type="file" accept="application/pdf" onChange={e => setFile(e.target.files[0])} className="sr-only" />
        {file && (
      ...
      {/* It will stack buttons on mobile and put them side-by-side on larger screens */}
      <div className="pt-4 flex flex-col-reverse md:flex-row md:justify-between items-center gap-4">
        <button
          onClick={onBack}
          disabled={loading}
          // RESPONSIVE CHANGE: Full width on mobile, auto on desktop
          className="w-full md:w-auto px-5 py-2.5 text-sm font-medium text-gray-300 rounded-xl hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 transition disabled:opacity-50"
        >
          ← Back
        </button>
        <button
          ...
          disabled={loading}
          // RESPONSIVE CHANGE: Full width on mobile, auto on desktop
          className={`w-full md:w-auto px-8 py-2.5 rounded-xl font-semibold text-white shadow-md transition-all duration-200 ${
            loading
